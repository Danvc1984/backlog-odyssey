      ...
      return (
        <Sheet open={openMobile} onOpenChange={setOpenMobile} {...props}>
          <SheetContent
            ...
            data-mobile="true"
            className="w-[--sidebar-width] bg-background p-0 text-foreground [&>button]:hidden z-[9999]"
            style={
              {
...
>(({ className, onClick, ...props }, ref) => {
  ...
  return (
    <Button
      ...
      variant="ghost"
      size="icon"
      className={cn("h-7 w-7 hover:bg-accent/90", className)}
      onClick={(event) => {
        onClick?.(event)
        toggleSidebar()
      }}
  ...
  return (
    <button
      ...
      className={cn(
        ...
        "group-data-[collapsible=icon]:w-full group-data-[collapsible=icon]:cursor-pointer",
        className
      )}
      {...props}
    />
...
>(({ className, ...props }, ref) => {
  return (
    <div
      ref={ref}
      data-sidebar="header"
      className={cn("flex flex-col gap-2 p-2 z-10", className)}
      {...props}
    />
  )
})
...
>(({ className, ...props }, ref) => {
  return (
    <div
      ref={ref}
      data-sidebar="body"
      className={cn("relative flex h-full flex-1 flex-col", className)}
      {...props}
    />
  )
})
...
>(({ className, ...props }, ref) => {
  return (
    <div
      ...
      className={cn(
        "flex min-h-0 flex-col gap-2 overflow-y-auto overflow-x-hidden z-10",
        className
      )}
      {...props}
    />
...
const sidebarMenuButtonVariants = cva(
  "peer/menu-button flex w-full items-center overflow-hidden rounded-md p-2 text-left text-sm outline-none ring-ring transition-[width,height,padding] focus-visible:ring-2 active:bg-accent/90 active:text-accent-foreground disabled:pointer-events-none disabled:opacity-50 group-has-[[data-sidebar=menu-action]]/menu-item:pr-8 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-[active=true]:font-medium data-[state=open]:hover:bg-accent/90 data-[state=open]:hover:text-accent-foreground group-data-[collapsible=icon]:!size-8 group-data-[collapsible=icon]:!p-2 group-data-[state=expanded]:gap-3 group-data-[collapsible=icon]:justify-center [&>span:last-child]:truncate [&>svg]:size-4 [&>svg]:shrink-0",
  {
    variants: {
      variant: {
        default: "hover:bg-accent/90",
        outline:
          "bg-background shadow-[0_0_0_1px_hsl(var(--border))] hover:bg-accent/90 hover:text-accent-foreground hover:shadow-[0_0_0_1px_hsl(var(--primary))]",
      },
      size: {
        default: "h-8 text-sm",
        sm: "h-7 text-xs",
